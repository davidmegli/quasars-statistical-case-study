...
\newgeometry{top=2cm, bottom=2cm, left=2cm, right=2cm} % Imposta i margini per la title page
\begin{titlepage}
   \begin{center}
       \vspace*{1cm}
...
<<setup_dataset, echo=FALSE, cache=FALSE>>=
require(astrodatR)
data("SDSS_QSO")
dat <- SDSS_QSO
...
\begin{itemize}
  ...
  \item Studio tramite modelli grafici per comprendere le relazioni fra le bande fotometriche e il redshift.
...
\end{itemize}
Le magnitudini, o luminosità, rappresentano la quantità di radiazione elettromagnetica emessa dal quasar in ciascuna banda. I valori delle magnitudini sono misurati in unità di magnitudine, una scala logaritmica in cui un aumento di 1 corrisponde a un decremento di luminosità di un fattore 2.5. Quindi un valore di magnitudine più basso indica una maggiore luminosità dell'oggetto. 
...
<<summary_dataset, echo=TRUE, cache=FALSE>>=
summary(dat)
@
Di seguito è riportato il boxplot relativo alle magnitudini nelle diverse bande dello spettro elettromagnetico (u, g, r, i, z), a cui sono stati rimossi i valori nulli.\\
<<setup_boxplot_errori_magnitudini1, echo=FALSE, cache=FALSE>>=
library(dplyr)
library(tidyr)
@
<<boxplot_magnitudini, echo=TRUE, fig.width=8, fig.height=5, cache=FALSE, dpi=300>>=
dat_long <- dat %>%
  pivot_longer(
...
@
Dai boxplot possiamo osservare che i valori sono piuttosto concentrati intorno alla mediana, seppur sono presenti molti outliers. Inoltre le bande u e g hanno valori massimi molto più altre delle altre bande.
Di seguito sono invece riportati i boxplot relativi agli errori di misura associati alle magnitudini nelle diverse bande dello spettro elettromagnetico (u, g, r, i, z).\\
<<setup_boxplot_errori_magnitudini2, echo=FALSE, cache=FALSE>>=
library(dplyr)
library(tidyr)
@
<<boxplot_errori_magnitudini, echo=FALSE, fig.width=8, fig.height=5, cache=FALSE, dpi=300>>=
dat_sig_long <- dat %>%
  pivot_longer(
...
ggplot(dat_sig_long, aes(x = banda, y = errore)) +
  geom_boxplot(fill = "orangered", color = "red") +
  labs(
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
@
Dai boxplot degli errori possiamo osservare che gli errori di misura sono piuttosto variabili, con valori massimi molto diversi tra le diverse bande. In particolare, l'errore di misura per la banda u sembra essere più elevato rispetto alle altre bande.
In una prima fase sperimentale è stato costruito il seguente modello lineare in cui il redshift è spiegato dalle magnitudini nelle diverse bande e dalle luminosità nelle bande radio e X:

<<linear_model, echo=TRUE, cache=FALSE>>=
...
\section{Correlazione fra Bande Fotometriche}
È stata calcolata la matrice di correlazione tra le variabili del dataset, per meglio comprendere le relazioni esistenti.

<<correlation_matrix, echo=TRUE, cache=FALSE>>=
...
\begin{itemize}
  ...
  \item Correlazione negativa tra z e Mp: Il coefficiente di correlazione tra z e Mp è -0.79, che indica una forte correlazione negativa. All'aumentare di z (redshift), il valore di Mp (magnitudine assoluta) tende a diminuire. Questo è coerente con il fatto che quasar più distanti (con redshift maggiore) sono più luminosi intrinsecamente (magnitudine assoluta minore). In generale, i quasar più lontani tendono ad essere più luminosi perché i quasar sono alimentati da buchi neri supermassicci al centro di galassie lontane e in rapida crescita. Quando guardiamo quasar molto distanti, stiamo osservando l'universo com'era miliardi di anni fa, e durante quel periodo, può essere che le galassie che ospitano questi quasar fossero più attive e brillanti rispetto a quelle che osserviamo più vicine a noi.
...
\end{itemize}
Per analizzare più in dettaglio le relazioni fra le bande fotometriche, prendiamo come esempio le bande i e z ed andiamo a visualizzare lo scatter plot che mette in relazione le magnitudini nelle due bande.
<<plot i_mag z_mag, echo=TRUE, cache=FALSE, fig.width=8, fig.height=5, cache=FALSE, dpi=300>>=
datno0 <- dat[dat$u_mag!=0,]
ggplot(datno0, aes(x=i_mag, y=z_mag)) +
  geom_point(alpha=0.5, color="brown3") +
  labs(
    title="Relazione tra i_mag e z",
    x="Magnitudine i",
    y="Redshift (z)"
  ) +
  theme_minimal()
@
Come si vede, la relazione tra i e z è praticamente lineare, andiamo a verificarlo costruendo un modello lineare che spieghi z in funzione di i.
<<linear model i z, echo=TRUE, cache=FALSE>>=
iz_lm <- lm(i_mag ~ z_mag, data = dat)
summary(iz_lm)
@
Come si nota dal P-value prossimo allo 0, e dal valore di R squared di 0,97, molto vicino ad 1, il modello lineare spiega molto bene la relazione tra i e z. Questo è coerente con quanto osservato nello scatter plot, che mostra una relazione lineare tra le due variabili.
\section{Distribuzioni}
È stata riportata qui di seguito la distribuzione della banda i, per avere un'idea della distribuzione delle magnitudini nella banda i e degli eventuali outliers.\\
<<Distribuzione di i_mag, echo=TRUE, fig.width=8, fig.height=5, cache=FALSE, dpi=300>>=
ggplot(dat, aes(x = i_mag)) +
  geom_histogram(bins = 50, fill = "brown3", color = "black") +
  labs(
    title = "Distribuzione di i_mag (magnitudine banda i)",
    x = "i_mag",
    y = "Conteggio"
  )
@
Nelle bande u, g, r, i, z sono presenti molti outliers, con valori nulli, nella distribuzione della banda i ne possiamo vedere alcuni sulla sinistra. Questi outliers potrebbero essere dovuti a errori di misurazione o a particolari proprietà fisiche dei quasar. Inoltre, la presenza di valori nulli potrebbe indicare che alcune osservazioni non sono state rilevate o sono incomplete.
...
<<Distribuzione del Redshift, echo=TRUE, fig.width=8, fig.height=5, cache=FALSE, dpi=300>>=
ggplot(dat, aes(x = z)) +
...
\\La presenza di una coda lunga verso valori più elevati di redshift (z > 3) indica che esistono alcuni quasar molto distanti, ma sono relativamente rari rispetto a quelli con z compresi tra 1 e 2. Questo può significare che i quasar più lontani sono più difficili da osservare.
...
\section{Relazione fra Redshift ed Errori}
<<rel red err setup, echo=FALSE, cache=FALSE>>=
...
@
<<relazione redshift errori, echo=TRUE, fig.width=8, fig.height=5, cache=FALSE, dpi=300>>=
#Correlazione fra redshift ed errori di misura
correlation <- cor(dat_num_only)
correlation_z <- correlation["z", c("sig_u_mag", "sig_g_mag",
                      "sig_r_mag", "sig_i_mag", "sig_z_mag")]
print(correlation_z)
# Plot della correlazione
barplot(correlation_z, col = "blue",
        main = "Correlazione tra Redshift e Errori di Misura",
        xlab = "Errori di Misura", ylab = "Correlazione")
@
Questo grafico mostra la correlazione tra il redshift (z) e gli errori di misura nelle diverse bande fotometriche (sig\_u\_mag, sig\_g\_mag, sig\_r\_mag, sig\_i\_mag, sig\_z\_mag). L'errore nella banda ultravioletta (sig\_u\_mag) ha la correlazione più elevata con il redshift (z), pari a circa 0,55. Questo potrebbe indicare che aumentando il redshift (quindi la distanza dell'oggetto), gli errori nelle osservazioni in banda u crescono sensibilmente. Ciò può essere dovuto al fatto che la banda u misura lunghezze d'onda ultraviolette, che diventano più deboli e difficili da rilevare con l'aumentare della distanza (effetto redshift cosmologico). Nelle bande ottiche più rosse (r, i, z), l'effetto del redshift sugli errori è meno pronunciato.

\section{Relazione fra Redshift e Bande Fotometriche}
Di seguito sono riportati alcuni scatter plot relativi alla relazione tra il redshift e le magnitudini nelle diverse bande dello spettro, nonché alla luminosità nelle bande radio e X. Queste visualizzazioni permettono di esplorare le relazioni fra le variabili e di individuare eventuali pattern o tendenze nei dati. In particolare sono stati scelti grafici che mettono in relazione il redshift con le magnitudini nelle bande u (ultravioletto), g (verde), r (rossa), i (infrarosso), z (further red), nonché con la luminosità nelle bande radio (FIRST) e X-ray (ROSAT), per capire se sussiste una relazione con il redshift.
<<setupdataset, echo=FALSE, cache=FALSE>>=
datno0 <- dat[dat$u_mag!=0,]
dat_rosat <- dat[dat$ROSAT>-9,]
dat_first <- dat[dat$FIRST!=-1,]
dat_first <- dat_first[dat_first$FIRST!=0,]
library(ggplot2)
@
<<Relazione tra Redshift e Magnitudine nella banda X-ray, echo=TRUE, fig.width=8, fig.height=5, cache=FALSE, dpi=300>>=
# Scatter plot della magnitudine ROSAT (X-ray)
ggplot(dat_rosat, aes(x = z, y = ROSAT)) +
  ...
  theme_minimal()
@
<<Relazione tra Redshift e Magnitudine nella banda u (ultraviolet), echo=TRUE, fig.width=8, fig.height=5, dpi=300>>=
# Scatter plot della magnitudine u
ggplot(datno0, aes(x = z, y = u_mag)) +
  ...
  theme_minimal()
@
<<Relazione tra Redshift e Magnitudine nella banda g (green), echo=TRUE, fig.width=8, fig.height=5, dpi=300>>=
# Scatter plot della magnitudine g
ggplot(datno0, aes(x = z, y = g_mag)) +
  ...
  theme_minimal()
@
<<Relazione tra Redshift e Magnitudine nella banda r (red), echo=TRUE, fig.width=8, fig.height=5, dpi=300>>=
# Scatter plot della magnitudine r
ggplot(datno0, aes(x = z, y = r_mag)) +
  ...
  theme_minimal()
@
<<Relazione tra Redshift e Magnitudine nella banda i (further red), echo=TRUE, fig.width=8, fig.height=5, dpi=300>>=
# Scatter plot della magnitudine i
ggplot(datno0, aes(x = z, y = i_mag)) +
  geom_point(alpha = 0.5, color = "brown3") +
  labs(
  ...
  theme_minimal()
@
<<Relazione tra Redshift e Magnitudine nella banda z (further red), echo=TRUE, fig.width=8, fig.height=5, dpi=300>>=
# Scatter plot della magnitudine z
ggplot(datno0, aes(x = z, y = z_mag)) +
  ...
  theme_minimal()
@
<<Relazione tra Redshift e Magnitudine nella banda radio, echo=TRUE, fig.width=8, fig.height=5, dpi=300>>=
# Scatter plot della magnitudine FIRST (radio)
ggplot(dat_first, aes(x = z, y = FIRST)) +
...
% <<linear model 1, echo=TRUE, cache=FALSE>>=
% a <- lm(z ~ u_mag + g_mag + r_mag + i_mag + z_mag + first + rosat,
%         data = dat)
% summary(a)
% @
...
library(GGally)
ggpairs(data.frame(u_mag, g_mag, r_mag, i_mag, z_mag, z))
@
...
\end{itemize}
\chapter{Analisi degli Outliers}
...
\section{Identificazione degli Outliers con Z-score}
In questa sezione andiamo a calcolare lo Z-score per ciascuna colonna delle bande fotometriche (u,g,r,i,z) e identifichiamo gli outlier come le osservazioni in cui il valore assoluto dello Z-score è maggiore di 3, ovvero distanti più di 3 deviazioni standard dalla media. Questo ci permetterà di individuare le osservazioni che si discostano significativamente dalla media e di confrontarle con i dati normali.
<<stampa degli outliers, echo=TRUE, cache=FALSE>>=
...
print("per outlier e dati normali:")
...
<<plot degli errori medi, echo=FALSE, fig.width=8, fig.height=5, cache=FALSE, dpi=300>>=
...
\section{Identificazione degli Outliers tramite banda i}
Come descritto nell'appendice C del libro "Modern Statistical Methods for Astronomy With R Applications" le rilevazioni dei quasar sono considerate complete per le magnitudini più brillanti, ovvero per magnitudine i fino a 19, mentre sono incomplete per magnitudini più deboli. Questo significa che i quasar più lontani o meno luminosi potrebbero non essere rilevati o essere soggetti a errori di misurazione.\\Per verificare questa ipotesi, andiamo a classificare gli outlier come quasar con magnitudine in banda i < 19 e confrontarli con i dati normali.
<<classificazione degli outlier, echo=TRUE, cache=FALSE>>=
...
print(error_means_outliers / error_means_normal)
@
<<plot degli errori medi 2, echo=FALSE, fig.width=8, fig.height=5, cache=FALSE, dpi=300>>=
# Visualizzazione della differenza con un grafico
library(ggplot2)
library(dplyr)
library(tidyr)
...
ggplot(error_long, aes(x = Banda, y = Errore, fill = Tipo)) +
  ...
  theme_minimal()
@
Anche in questo caso l'errore medio per gli outlier è più alto rispetto ai dati normali, con un rapporto di circa 1,4 per le bande g,r,i, 2 volte per la banda z e 3 volte per la banda u. Questo suggerisce che gli outlier con magnitudine i >= 19 presentano errori di misura più elevati rispetto ai dati normali, confermando l'ipotesi che i quasar più lontani o meno luminosi siano soggetti a errori di misurazione. In particolare, anche in questo caso gli outlier nella banda u (ultravioletto) sembrano essere quelli con gli errori di misura più elevati, suggerendo nuovamente che le misurazioni in questa banda siano più soggette a incertezze rispetto alle altre bande.
...
\chapter{Analisi delle Relazioni fra Variabili}
In questo capitolo verranno esplorate le relazioni tra le variabili del dataset attraverso rappresentazioni grafiche e analisi quantitative. In particolare, si cercherà di individuare eventuali associazioni tra il redshift e le magnitudini nelle diverse bande fotometriche, nonché tra il redshift e le luminosità nelle bande radio e X. Questo per cercare di comprendere meglio le proprietà dei quasar e le relazioni fra le loro caratteristiche fisiche.
...
<<rimozione degli esempi con valori mancanti, echo=TRUE, cache=FALSE>>=
...
<<Grafo Non Diretto, Stepwise, Forward, con AIC, echo=TRUE, cache=FALSE, fig.width=8, fig.height=7, dpi=300>>=
library(gRbase)
library(gRain)
library(gRim)
...
plot(graph, vertex.label = aic_dat_sw_f$varNames,
     main = "Grafo Non Diretto, Stepwise, Forward, con AIC"
     ,vertex.size=30,edge.width=2,vertex.labels=labels)

bic_dat_sw_f <- stepwise(sat_dat, k=log(nrow(dat_num_only)),
                         direction="forward")
bic_dat_sw_f
# Accesso alla componente del grafo (modelinfo$ug)
graph <- bic_dat_sw_f$modelinfo$ug
plot(graph, vertex.label = bic_dat_sw_f$varNames,
     main = "Grafo Non Diretto, Stepwise, Forward, con BIC"
     ,vertex.size=30,edge.width=2,vertex.labels=labels)
@
...
\\Notiamo anche che gli errori sono molto legati alle magnitudini corrispondenti, mostrando una dipendenza della misurazione dall'errore di misura, ma anche alle altre magnitudini e agli altri errori. Una possibile ipotesi potrebbe essere quindi che le misurazioni delle bande fotometriche siano state effettuate con lo stesso strumento, e suddivise in un secondo momento.
\\Proviamo per completezza a costruire un modello con l'algoritmo Stepwise Backward.
<<Grafo Non Diretto, Stepwise, Backward, con AIC, echo=TRUE, cache=FALSE, fig.width=8, fig.height=7, dpi=300>>=
bic_dat_sw_b <- stepwise(sat_dat, k=log(nrow(dat_num_only)),
                         direction="backward")
bic_dat_sw_b
# Accesso alla componente del grafo (modelinfo$ug)
graph <- bic_dat_sw_f$modelinfo$ug
plot(graph, vertex.label = bic_dat_sw_b$varNames,
...
<<Grafo Non Diretto, Stepwise, Forward, con AIC, senza errori, echo=TRUE, cache=FALSE, fig.width=8, fig.height=7, dpi=300>>=
...
plot(graph, vertex.label = colnames(bic_dat_sw_b2),
     ...
     vertex.labels=colnames(bic_dat_sw_b2),edge.curved=0)
@
Rispetto al grafo precedente (con tutte le variabili), questo grafo mostra una riduzione del numero di archi (connessioni) tra i nodi, il metodo Stepwise Backward ha eliminato le connessioni meno significative in termini di ottimizzazione del BIC, mantenendo solo quelle che contribuiscono maggiormente alla parsimonia del modello. Il Redshift rimane collegato alle bande fotometriche, questo suggerisce che il redshift ha ancora forti relazioni con le luminosità osservate nelle diverse bande fotometriche.
\\Le bande u\_mag, g\_mag, r\_mag, i\_mag, z\_mag continuano ad essere connesse fra loro, indicando forti correlazioni tra le magnitudini osservate nelle diverse lunghezze d'onda.
\\FIRST (radio) ha connessioni con meno variabili rispetto al modello completo, suggerendo una minor influenza diretta nel contesto ottico-X-ray rispetto alle bande fotometriche. Tuttavia questo potrebbe essere dovuto a una diversa scala di misura o strumento di misurazione.
\chapter{Modello predittivo del Redshift}
In questo capitolo verranno cercate ed analizzate eventuali dipendenze fra il redshift e le altre variabili presenti nel dataset. In particolare, si cercherà di costruire un modello predittivo per il redshift basato sulle magnitudini nelle diverse bande e sulle luminosità nelle bande radio e X. L'obiettivo è di valutare se le proprietà fotometriche e spettrali dei quasar possano essere utilizzate per predire il redshift, che è una misura della distanza e della velocità di allontanamento delle sorgenti.
\section{Divisione del Dataset}
...
<<funzione per valutazione del modello, echo=TRUE, cache=FALSE, fig.width=8, fig.height=7>>=
library(Metrics)
evaluate_model <-function(model, test_data, model_name) {
  ...
  ggplot() +
    geom_point(aes(x = test_data$z, y = predictions),
               color = "black", alpha = 0.5) +
    geom_abline(slope = 1, intercept = 0, color = "red",
                linetype = "dashed") +
    labs(title = paste("Redshift: Valori Reali vs Predetti. Modello:",
                       model_name),
         x = "Redshift Reale",
         y = "Redshift Predetto") +
    theme_minimal()
...
Per costruire un modello predittivo per il redshift proviamo ad utilizzata una regressione lineare multipla che spiega il redshift in funzione delle magnitudini nelle bande fotometriche. Questo modello permette di valutare l'importanza relativa delle variabili predittive e di stimare i coefficienti di regressione associati a ciascuna di esse.
<<modello di regressione lineare, echo=TRUE, cache=FALSE, dpi=300>>=
# Modello di regressione lineare
lm_model <- lm(z ~ u_mag + g_mag + r_mag + i_mag + z_mag,
               data = train_data)
...
\\Tramite bnlearn si costruisce un modello DAG utilizzando l'algoritmo Hill-Climbing (hc) per l'apprendimento della struttura della rete bayesiana. Successivamente, si apprendono i parametri della rete utilizzando i dati di addestramento.
<<modello con DAG, echo=TRUE, cache=FALSE, dpi=300>>=
# Bayesian Network
library(bnlearn)
...
plot(model)
@
Vediamo qua sopra che il grafo appreso è molto denso, evidenzianto le forti dipendenze fra le variabili. Le variabili fotometriche principali (u\_mag, g\_mag, r\_mag, i\_mag, z\_mag) presentano numerose connessioni sia tra loro che con le rispettive incertezze (sig\_u\_mag, sig\_g\_mag, …), questo suggerisce nuovamente che le magnitudini nelle diverse bande sono fortemente correlate e che gli errori influenzano le grandezze misurate.\\Adesso andiamo a valutare il modello appreso, testandolo sui dati di test e valutandone le prestazioni tramite MAE, RMSE e R-squared.
<<setup val mod dag, echo=FALSE, cache=FALSE>>=
library(Metrics)
require(Metrics)
@
<<valutazione del modello con DAG, echo=TRUE, cache=FALSE, fig.width=8, fig.height=7, dpi=300>>=
# Uso della funzione predict per il nodo "z" usando la rete appresa
predictions <- predict(fitted_model, node = "z", data = test_data,
                       method = "bayes-lw")
mae <- mae(test_data$z, predictions)  # Mean Absolute Error
rmse <- rmse(test_data$z, predictions)  # Root Mean Square Error
r_squared <- R2(test_data$z, predictions) # R-squared
...
ggplot() +
  geom_point(aes(x = test_data$z, y = predictions), color = "black",
             alpha = 0.5) +
  geom_abline(slope = 1, intercept = 0, color = "red",
              linetype = "dashed") +
  labs(
  title = "Redshift: Valori Reali vs Predetti. Modello: Bayesian Network",
       x = "Redshift Reale",
       y = "Redshift Predetto") +
  theme_minimal()
@
Gli errori sono molto bassi, e R-squared piuttosto alto (0.8827), questo significa che questo modello DAG è in grado di spiegare bene la variazione del redshift dei quasar. Il grafico mostra la relazione tra i valori reali e predetti del redshift, con una linea rossa che rappresenta la relazione ideale (valori reali = valori predetti). I punti neri rappresentano i valori reali e predetti del redshift, e la loro distribuzione indica quanto bene il modello sia in grado di prevedere i valori osservati.
\\Notiamo che rispetto al modello lineare presentato prima il modello DAG ha prestazioni molto migliori.
\\Notiamo anche che alcune predizioni sono fuori dal range 0-5.
\section{Modello con algoritmo di Machine Learning: Random Forest}
In questa sezione viene addestrato un modello di Random Forest per predire il redshift dei quasar in funzione delle magnitudini nelle bande fotometriche e delle luminosità nelle bande radio e X. Il Random Forest è un algoritmo di apprendimento supervisionato che costruisce un insieme di alberi decisionali e combina le loro previsioni per ottenere una previsione più accurata. Questo modello è in grado di catturare relazioni non lineari e complesse tra le variabili, e di gestire grandi quantità di dati con molte variabili predittive.
\\Lo scopo è di valutare se il Random Forest è in grado di migliorare le prestazioni rispetto al modello di regressione lineare e al modello DAG, e di fornire una previsione più accurata del redshift dei quasar.
<<modello con Random Forest, echo=TRUE, cache=FALSE>>=
library(randomForest)
rf_model <- randomForest(z ~ ., data = train_data, ntree = 50)
print(rf_model)
@
Come si può notare dai valori di errore, da R-squared e dal plot, questo modello ha prestazioni migliori rispetto al modello di regressione lineare e al modello DAG, ma chiaramente non ci è utile per spiegare le relazioni tra le variabili.
<<plot modello con Random Forest, echo=FALSE, fig.width=8, fig.height=7, cache=FALSE, dpi=300>>=
evaluate_model(rf_model, test_data, "Random Forest")
@

% \section{Modello con Gradient Boosting Machine}
