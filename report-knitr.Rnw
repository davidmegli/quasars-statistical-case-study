...
\chapter{Analisi delle Relazioni}
...
\\Le bande u_mag, g_mag, r_mag, i_mag, z_mag continuano ad essere connesse fra loro, indicando forti correlazioni tra le magnitudini osservate nelle diverse lunghezze d'onda.
\\FIRST (radio) ha connessioni con meno variabili rispetto al modello completo, suggerendo una minor influenza diretta nel contesto ottico-X-ray rispetto alle bande fotometriche. Tuttavia questo potrebbe essere dobuto a una diversa scala di misura.
\chapter{Predizione del Redshift}
...
@
\section{Modello con Random Forest}
